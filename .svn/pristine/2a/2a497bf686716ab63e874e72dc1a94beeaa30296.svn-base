...
db.serialize(function()
{
	...
	// 2. Igra
	db.run("CREATE TABLE if not exists Igra(id INTEGER PRIMARY KEY, naziv TEXT NOT NULL, nazivSlike TEXT NULL, pravila TEXT NOT NULL, obrisana INTEGER DEFAULT 0,timska INTEGER DEFAULT 0, timskaUloge TEXT, pozadina TEXT)", function(err)
	{
		if(err)
		{
			console.error(err);	
	...
	// 9. Turnir
	db.run("CREATE TABLE if not exists Turnir(id INTEGER PRIMARY KEY, idTipaTurnira INTEGER NOT NULL, naziv TEXT NOT NULL, idIgre INTEGER NOT NULL, pocetakPrijava INTEGER NOT NULL, krajPrijava INTEGER NOT NULL, datumVremePocetka INTEGER NOT NULL, datumVremeZavrsetka INTEGER NOT NULL,trenutnoKolo INTEGER, brDozvoljenihIgraca INTEGER, meceviNapravljeni INTEGER DEFAULT 0, obrisan DEFAULT 0, FOREIGN KEY(idTipaTurnira) REFERENCES TipTurnira(id), FOREIGN KEY(idIgre) REFERENCES Igra(id))", function(err)
		{
			if(err)
			{
				console.error(err);
	...
	// 12. PrijaveZaTurnir
	db.run("CREATE TABLE if not exists PrijaveZaTurnir(id INTEGER PRIMARY KEY, idTurnira INTEGER NOT NULL, idTima INTEGER NOT NULL, usernameKorisnika TEXT NOT NULL, datumVremePrijave INTEGER NOT NULL, FOREIGN KEY(idTurnira) REFERENCES Turnir(id), FOREIGN KEY(idTima) REFERENCES Tim(id), FOREIGN KEY(usernameKorisnika) REFERENCES Korisnik(username))", function(err)
		{	
			if(err)
			{
				console.error(err);
