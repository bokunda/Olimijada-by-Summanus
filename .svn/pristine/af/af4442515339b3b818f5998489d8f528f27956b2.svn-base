...
exports.uzmiPodatkeOKorisniku = function(username,callback)
{
	var db = otvoriKonekciju();

	var sql = "SELECT id,username,password,email,ime,prezime,urlSlike,datumRegistracije,banovan,brojPobeda,brojPoraza FROM Korisnik WHERE username=?";
...
exports.UzmiSveTurnire = function(callback)
{
	...
	var sql = "SELECT t.id,t.naziv AS nazivTurnira, t.pocetakPrijava, t.krajPrijava, t.datumVremePocetka, t.datumVremeZavrsetka, i.naziv AS nazivIgre, i.id AS idIgre FROM Turnir t JOIN Igra i ON t.idIgre=i.id WHERE t.obrisan=0 ORDER BY t.datumVremePocetka DESC";
...
exports.UzmiSveTimoveKorisnikaZaIgru = function(idKorisnika, idIgre, callback)
{
	var sql = "SELECT * FROM Timovi WHERE idIgre=? AND idKorisnika=?";
...
{
	var sql = "select distinct m.id, m.trajanje, k.username 'korisnik1', (select username from Korisnik k2 join Tim b2 on b2.idKorisnika=k2.id where m.idTima2=b2.id) 'korisnik2', m.datumVremePocetka, m.rezultat1, m.rezultat2 from Mec m join Tim b on b.id=m.idTima1 join Korisnik k on k.id=b.idKorisnika where m.idTurnira=?";
	var db = otvoriKonekciju();
...
{
	var sql = "SELECT id,idTurnira, idTima1, idTima2, datumVremePocetka, trajanje, rezultat1, rezultat2,tokMeca FROM Mec WHERE idTurnira=? ";
	db = otvoriKonekciju();
...
{
	var sql = "SELECT i.* FROM Igra i JOIN Turnir t on t.idIgre = i.id WHERE t.id = ?";
	var db = otvoriKonekciju();
...
{
	var sql = "SELECT *, ((brojPobeda*10)-(brojPoraza*3)) AS 'poena' FROM Korisnik WHERE obrisan=0 ORDER BY ((brojPobeda*10)-(brojPoraza*3)) DESC";
...
exports.DajSveTimoveKorisnika = function(idKorisnika, callback)
{
	...
	var sql = "SELECT t.*, i.nazivSlike, i.naziv FROM Timovi t JOIN Igra i ON t.idIgre = i.id WHERE t.idKorisnika = ? AND t.brojIgraca > 1";
...
exports.PrijaviKorisnikaNaTurnir = function(prijava,callback)
{
	//console.log("jel poziva? database");
	...
	db.all(sql,prijava.idTurnira,prijava.idTima,prijava.usernameKorisnika,prijava.datumVremePrijave,function(err)
	{
		//zatvoriKonekciju(db);
...
exports.OtkaziPrijavu = function(usernameKorisnika,callback)
{
	////console.log("asfasfdafasfsafasfasfasfa");
	var sql = "DELETE FROM PrijaveZaTurnir WHERE usernameKorisnika=?";

	//console.log("username: " + usernameKorisnika);

	var db = otvoriKonekciju();

	db.run(sql,usernameKorisnika,function(err)
	{
		//zatvoriKonekciju(db);
...
exports.DajPrijaveZaTurnir = function(idTurnira,callback)
{
	...
	var sql = "SELECT * FROM PrijaveZaTurnir p JOIN Timovi b on p.idTima = b.id WHERE p.idTurnira = ?";
...
exports.DajSveAdmine = function(callback)
{
	...
	var sql = "SELECT * FROM Admin WHERE obrisan!=? AND username!='admin'";
	var obrisan = 1;
...
{
	var danasnjiDatum = Math.floor(Date.now());
...
{
	var sql = "SELECT m.datumVremePocetka FROM Mec m JOIN Timovi b on m.idTima1=b.id JOIN Korisnik k on b.idKorisnika=k.id WHERE k.username=? AND m.rezultat1>m.rezultat2";	//sve pobede kad je bio "domacin"
	var sql1 = "SELECT m.datumVremePocetka FROM Mec m JOIN Timovi b on m.idTima2=b.id JOIN Korisnik k on b.idKorisnika=k.id WHERE k.username=? AND m.rezultat2>m.rezultat1";	//sve pobede kad je bio "gost"
...
{
	var sql = "SELECT m.datumVremePocetka FROM Mec m JOIN Timovi b on m.idTima1=b.id JOIN Korisnik k on b.idKorisnika=k.id WHERE k.username=? AND m.rezultat1<m.rezultat2";	//svi porazi kad je bio "domacin"
	var sql1 = "SELECT m.datumVremePocetka FROM Mec m JOIN Timovi b on m.idTima2=b.id JOIN Korisnik k on b.idKorisnika=k.id WHERE k.username=? AND m.rezultat2<m.rezultat1";	//svi porazi kad je bio "gost"
