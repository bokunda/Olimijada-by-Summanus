...
export class KreiranjeTimovaComponent implements OnInit
{
	...
	ProveriValidnostKontrola(): boolean
	{
		if (this.FC_igra.valid && this.FC_nazivTima.valid)
		{
			for (let i = 0; i < this.FC_NizNazivaIgraca.length;i++)
			{
				if (!this.FC_NizNazivaIgraca[i].valid)
				{
					return false;
				}

				if (!this.FC_NizBotova[i].valid)
				{
					return false;
				}
			}
    ...
    napraviJSON(): void
    {
		if (this.ProveriValidnostKontrola())
		{
			let jsonObjekat =
			{
				...
				this.timService.DodajTim(odgUSR.id, this.FC_nazivTima.value, 'trebaSrediti.jbg', this.FC_igra.value, JSON.stringify(jsonObjekat), this.brojIgraca).subscribe(odg =>
				{
					...
					if(odg.status == 0)
					{
						if(this.cookieService.get("language") == 'sr')
						{
							swal({
								type: 'error',
								title: 'Tim sa istim nazivom vec postoji!',
						...
						else
						{
							swal({
								type: 'error',
								title: 'Team with the same name already exists!',
								showConfirmButton: false,
								timer: 2600
							});
						}
					...
					else if(odg.status == 1)
					{
						...
						else
						{
							swal({
								type: 'success',
								title: 'The team was successfully created!',
								showConfirmButton: false,
								timer: 2600
							});
						}
						...
						/*odg.tim.opisTima = JSON.stringify(odg.tim.opisTima);*/
					...
					else
					{
						...
						else
						{
							swal({
								type: 'error',
								title: 'Error: The team is not created!'
							});
						}
    ...
    dialogDodajBota(indeksMatSelekta): void
    {
		...
		dialogRef.afterClosed().subscribe(bot =>
		{
			...
			{
				this.FC_NizBotova[indeksMatSelekta].setValue('');
			}
