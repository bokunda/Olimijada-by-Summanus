...
function DaLiJeBanovan(req,res,next)
{
	tokenGenerator.verify(req.token, tajniKljuc, function(err, decoded)
	{
		...
		else
		{
			...
			dbPristup.DaLiJeBanovan(username,function(callback)
			{
				if(callback == 1)
				{
					res.sendStatus(403);
...
ruter.post('/registracija',urlencodedParser,function(req,res)
{
	...
	dbPristup.registracija(0,korisnik, function(status,hash)
	{
...
ruter.post('/admin/registracija',urlencodedParser,uzmiToken,function(req,res)
{
	//console.log('a');
	var korisnik = req.body.korisnik;
	...
	{
		//console.log("DA LI RADIS I ZASTO NE?");
		//console.log(dekodiranikorisnik);
		if (err || dekodiranikorisnik.admin == 0)
		{
			res.sendStatus(403);
		}
		else
		{
			// registracija korisnika/admina (provera da li vec postoji username i ubacivanje u bazu ako ne postoji)
			dbPristup.registracija(1,korisnik, function(status,idAdmina)
			{
				// vraca se status koji je vratila funkcija za registraciju iz modula za pristup bazi
				// status = 1 - registrovan (generise se i vraca token)
				// status = 0 - vec postoji username
				...
				res.json({status: status, id: idAdmina})
			});
		}
	})
...
ruter.post('/prijava',function(req,res)
{
	...
	dbPristup.prijava(0,korisnik,function(status)
	{
		// status = 1 - OK
...
ruter.post('/admin/prijava',function(req,res)
{
	////////console.log('proba');
	var korisnik = req.body.korisnik;

	// provera username-a i poassword-a korisnika/admina u bazi
	dbPristup.prijava(1,korisnik,function(status)
	{
		// status = 1 - OK
		// status = 0 - nema ga u bazi
		// status = -1 - greska
		...
		{
			// kreira se token "5d"
			tokenGenerator.sign({admin: 1, username: korisnik.username},tajniKljuc,{expiresIn: "5d"},function(err,token)
			{
				// vraca se token i status = 1 - OK
				res.json({status: status, token: token, username: korisnik.username});
			});
...
ruter.post('/uzmi-podatke',urlencodedParser,uzmiToken,DaLiJeBanovan,function(req,res)
{
	...
	tokenGenerator.verify(token,tajniKljuc,function(err,dekodiraniKorisnik)
	{
		if (err || dekodiraniKorisnik.admin==1)
		{
			res.sendStatus(403);
		}
		else
...
ruter.get('/korisnik/:username', function(req,res)
{
	...
	dbPristup.uzmiPodatkeOKorisniku(req.params.username,0,function(status)
	{
		res.json(status);	
	});
...
ruter.post('/korisnik',uzmiToken,DaLiJeBanovan,function(req,res)
{
	var token = req.token;
	...
	tokenGenerator.verify(token, tajniKljuc,function(err,dekodiraniKorisnik)	
	{
		...
		else
		{
			...
			dbPristup.uzmiPodatkeOKorisniku(username,1, function(status)
			{
				res.json(status);
			})
		}
...
ruter.post('/obrisiAdmina',uzmiToken,function(req,res)
{
	tokenGenerator.verify(req.token, tajniKljuc, function(err, dekodiraniKorisnik)
	{
		if (err || dekodiraniKorisnik.admin == 0 || dekodiraniKorisnik.username != "admin")
		{
			res.sendStatus(403);
		}
		else
...
ruter.get('/poslednjaDvaTurnira', function(req, res)
{
	dbPristup.UzmiPoslednjaDvaTurnira(function(turniri)
	{
		res.json(turniri);
	});
});
...
ruter.post('/rezultatiTurnira', function(req,res)
{
	...
	dbPristup.DajTurnirZaId(idTurnira,function(turnir)
	{
		...
		{
			dbPristup.DajIgruTurnira(idTurnira, function(igra)
			{
				if (igra == -1)
				{
					res.json(-1);
				...
				{
					dbPristup.DajRezultateTurnira(idTurnira, function(rezultati)
					{
						if (rezultati == -1)
						{
							res.json(rezultati);
...
ruter.post('/dodajBota',uzmiToken,DaLiJeBanovan, function(req,res)
{
	...
	tokenGenerator.verify(req.token,tajniKljuc, function(err,korisnik)
	{
		...
		else
		{
			...
			dbPristup.uzmiPodatkeOKorisniku(korisnik.username,0,function(status1)
			{
...
ruter.post('/uploadAvatara',uzmiToken,DaLiJeBanovan, function(req,res)
{
	tokenGenerator.verify(req.token,tajniKljuc, function(err,korisnik)
	{
		if (err || korisnik.admin == 1)
...
ruter.post('/dodajTim', uzmiToken,DaLiJeBanovan, function(req, res)
{
	tokenGenerator.verify(req.token, tajniKljuc, function(err,korisnik)
	{
		if(err || korisnik.admin == 1)
...
ruter.post('/dodajTurnir',uzmiToken, function(req,res)
{
	tokenGenerator.verify(req.token, tajniKljuc, function(err,dekodiraniKorisnik)
	{
		...
		else
		{
			...
			/*dbPristup.DaLiPostojiTurnir(req.body.naziv,function(status)
			{
				if(status == -1)
				{
					res.json({status: -1});
...
ruter.post('/:username/timovi', uzmiToken,DaLiJeBanovan, function(req,res)
{
	tokenGenerator.verify(req.token, tajniKljuc,function(err,korisnik)
	{
		if (err || korisnik.admin == 1)
		...
		else
		{
			...
			////////console.log("isti su");
			dbPristup.uzmiPodatkeOKorisniku(username,0,function(status)
			{
				if (status == -1)
				{
					res.json(status);
				...
				else
				{
					...
					{
						dbPristup.UzmiSveTimoveKorisnikaZaIgru(status.id,req.body.idIgre,igra.timska,function(status2)
						{
...
ruter.get('/:username/botovi',uzmiToken,DaLiJeBanovan, function(req,res)
{
...
ruter.post('/:username/botovi', uzmiToken,DaLiJeBanovan, function(req,res)
{
    ////////console.log("username/botovi");
	tokenGenerator.verify(req.token, tajniKljuc,function(err,korisnik)
	{
...
ruter.post("/obrisiBota",uzmiToken,DaLiJeBanovan, function(req,res)
{
	var token = req.token;
	tokenGenerator.verify(token, tajniKljuc,function(err,dekodiraniKorisnik)
	{
		...
		{
			dbPristup.uzmiPodatkeOKorisniku(dekodiraniKorisnik.username,0,function(korisnik)
			{
...
ruter.post('/izmeniBota',uzmiToken,DaLiJeBanovan,function(req,res)
{
	var token = req.token;
	tokenGenerator.verify(token,tajniKljuc,function(err,dekodiraniKorisnik)
	{
			...
			dbPristup.uzmiPodatkeOKorisniku(dekodiraniKorisnik.username, 0, function(korisnik)
			{
				...
				else
				{
					dbPristup.UzmiPodatkeOBotu(req.body.idBota,function(statusUzmiPodatke)
					{
						...
						else if (statusUzmiPodatke.idKorisnika != korisnik.id)
						{
							res.sendStatus(403);
						}
						else
...
// NAPOMENA ----- pre je bilo get, tako da ako ne radi nesto, mozda je zbog toga (sa tokenom)
ruter.post("/turniri/mecevi",function(req,res)
{
	console.log(req.body);
	var idTurnira = req.body.idTurnira;
	...
	//{
		...
		*/
		{
			dbPristup.UzmiNazivIgreITurniraZaTurnir(idTurnira, function(podaci)
			{
				if (podaci == -1)
				{
					res.json(-1);
				...
				{
					var odgovor = {
						igra: podaci.nazivIgre,
						turnir: podaci.nazivTurnira,
						mecevi: []
					}
					...
					dbPristup.UzmiSveMeceveSaTurnira(idTurnira,function(statusSviMecevi)
					{
						...
						odgovor.mecevi = statusSviMecevi;
						res.json(odgovor);
					})
				}
			})
...
// bez tokena
ruter.post('/meceviNaTurniru',function(req,res)
{
	var idTurnira = req.body.idTurnira;

	dbPristup.UzmiSveMeceveZaPrikazZaSelTurnir(idTurnira,function(mecevi)
	{
		res.json(mecevi);
	})
})

ruter.post("/dajFormu", function(req, res)
{
	dbPristup.DajFormuKorisnika(req.body.usernameKorisnika, function(statusForma)
	{
		if(statusForma == -1)
		{
			res.json([]);
		...
		{
			console.log(statusForma);
			res.json(statusForma);
		}
	});
});

// ruta za dobijanje svih meceva korisnika ciji je username = username
ruter.get("/:username/mecevi",uzmiToken,DaLiJeBanovan, function(req,res)
{
	var username = req.params.username;
	var token = req.token;
	...
	tokenGenerator.verify(token,tajniKljuc,function(err,dekodiraniKorisnik)
	{
		...
		{
			var username = dekodiraniKorisnik.username;
				dbPristup.uzmiPodatkeOKorisniku(username,0,function(statusKorisnik)
				{
					if (statusKorisnik == -1)
					{
...
ruter.post('/dodajFaq', uzmiToken, function(req, res)
{
	tokenGenerator.verify(req.token, tajniKljuc, function(err,dekodiraniKorisnik)
	{
		...
		else
		{
			...
			dbPristup.dodajPitanjeOdgovorZaFaq(req.body.pitanje, req.body.odgovor, function(odgovor, idPitanja)
			{
				if (odgovor == -1)
				{
					...
					res.json({status: odgovor});
				}
				else
				{
					res.json({status: odgovor, id: idPitanja});
				}
			});
		}
	});
...
ruter.post('/dajSveMatches2',function(req,res)
{

	var username = req.body.username;

	dbPristup.DajSveMatches2(username,function(korisnici)		
	{
		//////console.log(korisnici);
		res.json(korisnici);
	});
...
ruter.post('/dajTimZaBota', uzmiToken,DaLiJeBanovan, function(req,res)
{
	tokenGenerator.verify(req.token, tajniKljuc, function(err,dekodiraniKorisnik)
	{
		if (err || dekodiraniKorisnik.admin == 1)
		...
		{
			dbPristup.UzmiTimZaBota(req.body.idBota, function(tim)
			{
				res.json(tim);
			})
		}
...
ruter.post('/admin/dajSveKorisnike',uzmiToken,function(req,res)
{
	tokenGenerator.verify(req.token, tajniKljuc, function(err,dekodiraniKorisnik)
	{
		if (err || dekodiraniKorisnik.admin == 0)
		...
		{
			dbPristup.DajSveKorisnikeAdmin(function(korisnici)
			{
				res.json(korisnici);
			})
		}
...
ruter.post('/dajSveTimoveKorisnika', uzmiToken,DaLiJeBanovan, function(req,res)
{
	...
	tokenGenerator.verify(req.token, tajniKljuc, function(err,dekodiraniKorisnik)
	{
		...
		else
		{
			dbPristup.uzmiPodatkeOKorisniku(dekodiraniKorisnik.username,0, function(korisnik)
			{
				...
				{
					dbPristup.DajSveTimoveKorisnika(korisnik.id, function(timoviKorisnika)
					{
...
ruter.post('/banujKorisnika', uzmiToken, function(req,res)
{
	...
	tokenGenerator.verify(req.token, tajniKljuc, function(err,dekodiraniKorisnik)
	{
		...
		else
		{
			...
			dbPristup.uzmiPodatkeOKorisniku(korisnik.username, 1,function(korisnik1)
			{
				if (korisnik != -1)
				{
					...
					mail.sendMail(korisnik1.email,subject,korisnik.banPoruka);
				}
...
ruter.post('/prijaviKorisnikaNaTurnir', uzmiToken,DaLiJeBanovan, function(req,res)
{
	////////console.log("idturnira: " + req.body.id);
	//var id = req.body.id;
	...
	tokenGenerator.verify(req.token, tajniKljuc, function(err,dekodiraniKorisnik)
	{
		...
		else
		{
			...
			dbPristup.UzmiPodatkeOTimu(req.body.idTima,function(tim)
			{
				if (tim == -1)
				{
					res.json(-1);
				...
				else
				{
					dbPristup.uzmiPodatkeOKorisniku(dekodiraniKorisnik.username,0,function(korisnik)
					{
						...
						{
							if (tim.idKorisnika != korisnik.id)
							{
								res.sendStatus(403);
							}
							else
							{
								dbPristup.PrijaviKorisnikaNaTurnir(req.body,dekodiraniKorisnik.username,function(callback)
								{
									res.json(callback);
								});
							}
...
// jelena
ruter.post('/testirajTim', uzmiToken,DaLiJeBanovan, function(req,res)
{
	var token = req.token;
    ...
    tokenGenerator.verify(token, tajniKljuc, function(err,dekodiraniKorisnik)
    {
        ...
        else
        {
			dbPristup.uzmiPodatkeOKorisniku(dekodiraniKorisnik.username,0,function(korisnik)
			{
				...
				{
					dbPristup.UzmiPodatkeOTimu(idTima,function(tim)
					{
						if (tim == -1)
						{
							res.json(tim);
						...
						else
						{
							...
							else
							{
								dbPristup.UzmiTipMeca("Testiranje",function(tipMeca)
								{
									...
									{
										dbPristup.DajRandomTim(tim.idIgre,function(idRandomTima)
										{
											if (idRandomTima == -1)
											{
												res.json(idRandomTima);
											}
											else
											{
												if (idRandomTima == 0)
												...
												dbPristup.UnesiMecZaTestiranje(idTima,idRandomTima,tipMeca.id,function(statusUnet,idUnet)
												{
...
ruter.post('/testirajBota', uzmiToken,DaLiJeBanovan, function(req,res)
{
    var token = req.token;
    ...
    tokenGenerator.verify(token, tajniKljuc, function(err, dekodiraniKorisnik)
    {
        ...
        else
        {
			dbPristup.uzmiPodatkeOKorisniku(dekodiraniKorisnik.username,0, function(korisnik)
			{
				...
				{
					dbPristup.UzmiPodatkeOTimuBota(idBota,function(bot)
					{
						if (bot == -1)
						{
							//console.log('bot');
						...
						{
							if (bot.idKorisnika != korisnik.id)
							{
								res.sendStatus(403);
							}
							else
							{
								dbPristup.UzmiTipMeca("Testiranje",function(tipMeca)
								{
									...
									{
										dbPristup.DajRandomTim(bot.idIgre,function(idRandomBota)
										{
											if (idRandomBota == -1)
											{
											...
											else
											{
												...
												//////console.log("tip meca id: ",tipMeca.id)
												dbPristup.UnesiMecZaTestiranje(bot.id,idRandomBota,tipMeca.id,function(statusUnet,idUnet)
												{
...
ruter.post('/obrisiTim', uzmiToken,DaLiJeBanovan, function(req,res)
{
	var token = req.token;
	var idTima = req.body.idTima;
				...
				else
				{
					dbPristup.UzmiPodatkeOTimu(idTima,function(tim)
					{
						...
						else
						{
							if (tim.idKorisnika == korisnik.id)
							{
								dbPristup.ObrisiTim(idTima,function(status)
								{
									res.json(status);
								})
							}
...
ruter.post('/otkaziPrijavuZaTurnir',uzmiToken,DaLiJeBanovan,function(req,res)
{
	...
	tokenGenerator.verify(req.token, tajniKljuc, function(err,decoded)
	{
		...
		else
		{
			console.log(idTurnira);
				...
				dbPristup.OtkaziPrijavu(decoded.username,idTurnira,function(callback)
				{
					res.json(callback);
				});
		}
...
ruter.post('/obrisiNalog', uzmiToken,DaLiJeBanovan, function(req,res)
{
...
ruter.post('/ukupanBrojPobedaZaKorisnika', function(req, res)
{
	var username = req.body.username;

	dbPristup.UkupanBrojPobedaZaKorisnika(username, function(niz)
	{
		res.json(niz);
	});
});

ruter.post('/ukupanBrojPorazaZaKorisnika', function(req, res)
{
	var username = req.body.username;

	dbPristup.UkupanBrojPorazaZaKorisnika(username, function(niz)
	{
		res.json(niz);
	});
});
...
// NIKOLA TOKEN -- OKEJ JELENA
ruter.post('/UkupanBrojMecevaPoMesecima',uzmiToken,function(req,res)
{
	tokenGenerator.verify(req.token,tajniKljuc, function(err,decoded)
	{
		...
		{		
			dbPristup.UkupanBrojMecevaPoMesecima(function(niz)
			{
				res.json(niz);
			});
		}
...
ruter.post('/nazivSlikeKorisnika', uzmiToken,DaLiJeBanovan, function(req,res)
{
...
ruter.post('/admin/obrisiNalog', uzmiToken, function(req, res)
{
	var token = req.token; 
	tokenGenerator.verify(token, tajniKljuc, function(err,dekodiraniKorisnik)
	{
		...
		{
			dbPristup.UzmiPodatkeOAdminu(dekodiraniKorisnik.username, function(admin)
			{
				if (admin == -1)
				{
					res.json(admin);
				...
				{
					dbPristup.ObrisiAdminaZaId(admin.id, function(status)
					{
						res.json(status);
					})
				}
...
ruter.post('/ukupanBrojPorazaZaKorisnkaPoMesecima',uzmiToken,DaLiJeBanovan,function(req,res)
{
	tokenGenerator.verify(req.token,tajniKljuc, function(err,decoded)
	{
		if (err || decoded.admin == 1)
		...
		{	
			...
			{
				console.log('niz poraza: ' + JSON.stringify(niz));
				res.json(niz);
			});
		}
						...
						else
						{
							...
							var httpreq = http.request(options, function(httpres)
							{
								...
								httpres.on('end',function()
								{
									...
									if (mec.rezultat1 == null || mec.rezultat2 == null)
									{
										dbPristup.UnesiRezultateMeca(idMeca,odgovor.rez1, odgovor.rez2, odgovor.tok, odgovor.tok.trajanje, function(status)
										{
...
{
	dbPristup.UzmiSveAktuelneIZakazaneMeceve(50,function(mecevi)
	{
		res.json(mecevi);
	})
})
...
ruter.post('/brojOsvojenihTurniraZaKorisnika',uzmiToken,DaLiJeBanovan,function(req,res)
{
	tokenGenerator.verify(req.token,tajniKljuc, function(err,decoded)
	{
		////////console.log("decoded: " + decoded);
...
ruter.post('/posaljiMailZaReset',function(req,res)
{
	var pom = req.body;
	//console.log(pom);
	var korisnik = pom.korisnik;
	//console.log("korisnik: " + korisnik);
	var kod = pom.kod;
	//console.log("kod: " + kod);

	dbPristup.DajMailZaUsername(korisnik,function(email)
	{
		if(email == -1)
		{
			res.json(-1);
		...
		{
			var poruka = "Verifikacioni kod: " + kod;
			var subject = "Olimijada - verifikacioni kod";
			mail.sendMail(email[0].email,subject,poruka);
			res.json(1);
		}
	})
...
ruter.post('/dajUsername',uzmiToken,DaLiJeBanovan,function(req,res)
{
	tokenGenerator.verify(req.token,tajniKljuc, function(err,decoded)
	{
		////////console.log("decoded: " + decoded);
...
ruter.post('/daLiIgraTimska',uzmiToken, function(req,res)
{
	...
	tokenGenerator.verify(req.token, tajniKljuc, function(err,dekodiraniKorisnik)
	{
		...
		{
			dbPristup.DaLiIgraTimska(req.body.id,function(odg)
			{
				res.json(odg);
			});
		}
